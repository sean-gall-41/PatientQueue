...
static const std::string OUTPUT_LOGS_FILE = "unit_tests.txt";


// function prototype declarations
static std::string randomString(int maxLength);
void test(PatientQueue& queue, Vector<std::string> &logs);
void vectorizeOutputLogs(Vector<string> &inVec);
...
int main() {
    ...
    Vector<std::string> outputLogs;
    vectorizeOutputLogs(outputLogs);
    while (true) {
        try {
            while (true) {
            ...
            std::cout << "The program exited unexpectedly" << std::endl;
            //break;
            //system("pause");
            //std::cout << "Exiting." << std::endl;
            //return 3;
        }
        if (!getYesOrNo("Would you like to run another queue (y/n)? ")) {
            break;
        }
    }
    std::cout << std::endl;
    std::cout << "Exiting successfully" << std::endl;
    system("pause"); // Get separate window from system, not sent to java backend...
    return 0;
}
...
void vectorizeOutputLogs(Vector<std::string> &inVec) {
    std::ifstream inputFile;
    inputFile.open(OUTPUT_LOGS_FILE);
    if (inputFile.fail()) throw std::runtime_error("Could not open input file");
    while (true) {
        std::string log;
        getline(inputFile, log);
        if (inputFile.fail()) break;
        if (!startsWith(log, '#')) inVec.add(log);
    }
}
 ...
 */
void test(PatientQueue& queue, Vector<string> &logs) {
    bool unitTest = getYesOrNo("Unit Test(Y/N)?");
    std::stringstream logParser;
    if (unitTest) { //Now get output front specific log
        Vector<std::string> outputLogs;
        vectorizeOutputLogs(outputLogs);
        int logNum = getInteger("Which output do you want to generate? ");
        std::string logToParse = logs[logNum-1]; // should range check here or above
        logParser << logToParse;
    }
    while (true) {
        std::cout << endl;
        ...
        if (unitTest) { //automated luxury gay space communism
            std::string token;
            if (logParser >> token) {
                std::cout << "N)ew, F)ront, U)pgrade, P)rocess, B)ulk, C)lear, Q)uit? "
                          << token << std::endl;
                ...
                if (token == "N") {
                    ...
                    logParser >> token;
                    std::cout << "Priority? " << token << std::endl;
                    int priority = stringToInteger(token);
                    queue.newPatient(name, priority);
                } else if (token== "F") {
                    ...
                    std::cout << "Name? " << name << std::endl;
                    logParser >> token;
                    std::cout << "New priority? " << token << std::endl;
                    int newPriority = stringToInteger(token);
                    queue.upgradePatient(name, newPriority);
                } else if (token == "P") {
                    std::string value = queue.processPatient();
                    std::cout << "Processing patient: \"" << value << "\"" << std::endl;
                } else if (token == "B") {
                    logParser >> token;
                    std::cout << "How many patients? " << token << std::endl;
                    int numPatients = stringToInteger(token);
                    logParser >> token;
                    std::string choice = toUpperCase(token);
                    std::cout << "N)ew or P)rocess: " << choice << std::endl;
                    if (choice == "N") {
                        bulkEnqueue(queue, numPatients);
                    } else if (choice == "P") {
                        bulkDequeue(queue, numPatients);
                    }
                } else if (token == "C") queue.clear();
                else if (token == "Q") break;
            } else break; //FIXME probably a bug here
            //TODO implement functionality to take in inputs from unit_tests.txt
            // and automatically receive input from a given line.
        } else { //manual user input
            ...
            if (choice.empty() || choice == "Q") {
                ...
                while (priority <= 0) {
                    std::cout << "The priority must be greater than zero. "
                              << "Try again.\n";
